...
const TIMES_VALUES = [
  688, 128, 68, 48, 38, 22, 20, 18, 16, 15, 14, 13, 12, 12, 12, 12, 13, 14, 15,
  16, 18, 20, 22, 38, 48, 68, 128, 688,
];
const INITIAL_PRICE = 1500;
const PLAN_PRICES = [5, 4, 3];

const get_current_lottery = (lottype) => {
  for (lottery of lotteries) if (lottery["lottype"] === lottype) return lottery;
  return null;
};
      ...
      if (lottype === "jianada28" || lottype === "taiwan28") {
        ...
        if (curData && lastData) {
          ...
          return {
            ...
            countTime: Number(
              (parseInt(curData["openTime"]) -
                parseInt(curData["serverTime"])) /
                1000
            ),
            preDrawTime: moment(lastData["openTime"])
              .utc()
              .add(8, "hours")
              .format("YYYY-MM-DD HH:mm:ss"),
            drawTime: moment(curData["openTime"])
              .utc()
              .add(8, "hours")
              .format("YYYY-MM-DD HH:mm:ss"),
            sumNum: sumNum,
        ...
        if (response.status === 200) {
          ...
          if ("result" in data) {
            ...
            if ("serverTime" in resData)
              resData["countTime"] = moment(resData["drawTime"]).diff(
                moment(resData["serverTime"]),
                "seconds"
              );
            return resData;
            ...
            if ("result" in curData) result = curData["result"].split(",");
            else result = data["body"][1]["result"].split(",");
            ...
            const resData = {
              ...
              countTime:
                60 -
                (Math.floor(Number(parseInt(data["timestamp"]) / 1000)) % 60),
              preDrawTime: moment(curData["timestamp"]).format(
                "YYYY-MM-DD HH:mm:ss"
              ),
              drawTime: moment(curData["timestamp"])
                .add(1, "minute")
                .format("YYYY-MM-DD HH:mm:ss"),
              sumNum: sumNum,
            ...
            if (!("result" in curData)) resData["countTime"] = 0;
...
const get_past_data = async (lottype, date, rows) => {
  try {
    ...
    const records = await PlanHistory.find(params)
      .sort({ preDrawIssue: -1 })
      .limit(rows);
    return records;
  } catch (err) {
  ...
  try {
    ...
    for (let k = pastData.length - 1; k >= 0; k--) {
      try {
        ...
        if (gameType === "sum3") bingoValue = _.sum(values);
        ...
        for (let i = 0; i < 3; i++) {
          ...
          // Create New Plan
          if (createPlan[i]) {
            planValues[i] = _.sampleSize(_.range(minValue, maxValue), 5 - i);
          ...
          if (planValues[i].includes(bingoValue)) {
            ...
            if (gameType == "sum3")
              profit[i] =
                Number(profit[i]) +
                (lotteryCost[i] / PLAN_PRICES[i]) * TIMES_VALUES[bingoValue];
            else
              profit[i] = (Number(profit[i]) + lotteryCost[i] - 0.01).toFixed(
                2
              );
          } else {
